...
namespace {
    ...
    struct SceneUniforms {
        ...
        veekay::mat4 dir_light_matrix;  // Матрица источника света для теней
    ...
    // Функция построения Look-At матрицы
    ...
    // Функция Look-At для источника света
    ...
    // Ортографическая матрица проекции для источника света
    ...
    // Переход состояния изображения для карты теней
    ...
    // Создание ресурсов для карты теней
    void createShadowMapResource(VkDevice device, VkImage &image, VkDeviceMemory &mem,
                                VkImageView &view_depth, VkImageView &view_sampled) {
        ...
        vkCreateImageView(device, &viewInfo, nullptr, &view_sampled);
        
        viewInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT;
        vkCreateImageView(device, &viewInfo, nullptr, &view_depth);
    }
    ...
    void initialize(VkCommandBuffer cmd) {
        ...
        // Основной графический пайплайн
        {
            ...
            // Лейаут для сцены (включая карту теней)
            {
    ...
    void update(double time) {
        ...
        // Построение матриц для карты теней
    ...
    // Проход рендера для карты теней
